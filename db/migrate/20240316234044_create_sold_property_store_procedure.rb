class CreateSoldPropertyStoreProcedure < ActiveRecord::Migration[7.1]
  ...
  def down
    execute <<-SQL
      DROP PROCEDURE IF EXISTS create_sold_property(name VARCHAR, city VARCHAR, address VARCHAR, sale_price NUMERIC, bedrooms INTEGER, has_pool BOOLEAN, sale_date DATE, agent_commission NUMERIC, buyer_id INTEGER, seller_id INTEGER, agent_id INTEGER);

      DROP PROCEDURE IF EXISTS update_sold_property(property_id INTEGER, new_name VARCHAR, new_city VARCHAR, new_address VARCHAR, new_sale_price NUMERIC, new_bedrooms INTEGER, new_has_pool BOOLEAN, new_sale_date DATE, new_agent_commission NUMERIC,buyer_id INTEGER, seller_id INTEGER, agent_id INTEGER);
